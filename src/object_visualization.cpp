...
void cbClosest(const pm_assign2::boundingConstPtr& msg)
{
    ...
    cv::putText(left_image, "X:" + std::to_string(msg->xp) + " Y:" + std::to_string(msg->yp) + " Z:" +std::to_string(msg->zp), cv::Point(msg->x, msg->y), cv::FONT_HERSHEY_DUPLEX,0.7,cv::Scalar(0,255,0),1, 2,false);
    cv::imshow("Left image", left_image);
    cv::waitKey(1);
